...
const PolaroidCard: React.FC<PolaroidCardProps> = ({ imageUrl, caption, status, error, dragConstraintsRef, onShake, onDownload, isMobile }) => {
    ...
    useEffect(() => {
        // Reset animation states whenever the image URL changes or status goes back to pending
        ...
        if (status === 'done' && imageUrl) {
            ...
            // This is the key: Check if the image is already loaded (e.g., from cache or base64)
            if (img.complete) {
                handleLoad();
            } else {
    ...
    // This effect controls the "developing" animation *after* the image has loaded.
    useEffect(() => {
        if (isImageLoaded) {
                ...
                {status === 'done' && imageUrl && (
                    <>
                        ...
                        <img
                            ...
                            // Hide the image until it is loaded and ready for the animation
